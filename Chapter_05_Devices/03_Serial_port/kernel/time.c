...
static void ktimer_schedule()
{
	...
	timespec_t time_monotonic, ref_time_monotonic;

	if (!sys__feature(FEATURE_TIMERS, FEATURE_GET, 0))
		return;

	kclock_gettime(CLOCK_REALTIME, &time);
	kclock_gettime(CLOCK_MONOTONIC, &time_monotonic);
	/* should have separate "scheduler" for each clock */

	ref_time = time;
	time_add(&ref_time, &threshold);
	ref_time_monotonic = time_monotonic;
	time_add(&ref_time_monotonic, &threshold);
	...
	while (first != NULL)
	{
		if(first->clockid == CLOCK_REALTIME) {
			...
			/* timers have absolute values in 'it_value' */
			if (time_cmp(&first->itimer.it_value, &ref_time_monotonic) <= 0)
			{
				/* 'activate' timer */
				...
				*/
				...
				if (next != NULL)
				{
					ref_time_monotonic = next->itimer.it_value;
					time_sub(&ref_time_monotonic, &time_monotonic);
					arch_timer_set(&ref_time_monotonic, ktimer_schedule);
				}
				/* evade this behaviour! */
				...
				if (first)
				{
					ref_time_monotonic = first->itimer.it_value;
					time_sub(&ref_time_monotonic, &time_monotonic);
					arch_timer_set(&ref_time_monotonic, ktimer_schedule);
				}
				break;
